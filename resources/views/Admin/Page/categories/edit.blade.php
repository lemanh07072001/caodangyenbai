...
{{--  Breadcrumbs  --}}
@section('breadcrumbs')
    {{ Breadcrumbs::render('CategoriesEdit',$getFind) }}
@endsection
...
@section('content')
    <section class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="{{route('categories.update',$getFind->id)}}">
                    @csrf
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select id="inputStatus" name="parent_id" class="form-control custom-select @error('parent_id')is-invalid @enderror">
                                    ...
                                    @if(!empty($getCategories))
                                        {{showCategoriesSelect($getCategories,old('parent_id')??$getFind->parent_id)}}
                                    @endif
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select id="inputStatus" name="type" class="form-control custom-select @error('type')is-invalid @enderror">
                                    <option value="" {{old('type')||$getFind->type===null?'selected':''}}>Chọn vị trí</option>
                                    <option value="0"{{old('type')||$getFind->type===0?'selected':''}}>Danh mục top</option>
                                    <option value="1"{{old('type')||$getFind->type===1?'selected':''}}>Danh mục chính</option>
                                </select>
